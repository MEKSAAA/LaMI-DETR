...
from lamidetr.layers import LayerNorm
...
class ConvNeXt(Backbone):
    ...
    def forward_features(self, x):
        ...
        outs_wonorm = {}
        for i in range(4):
            x = self.downsample_layers[i](x)# [2, 3, 736, 917]->[2, 192, 184, 229]
            x = self.stages[i](x)
            if i in self.out_indices:
                if self.score_ensemble:
                    outs_wonorm["p{}".format(i)] = x    
                    ...
                    outs["p{}".format(i)] = x_out
        if self.score_ensemble:
            return outs, outs_wonorm# score ensemble的时候without norm的feature效果更好
        else:
            return outs
